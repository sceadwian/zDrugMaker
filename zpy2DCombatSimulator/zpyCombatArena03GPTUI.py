"""
zpyCombatArena04 -- roster-driven top-down team combat sim (stdlib only).
...
class App:
    """Modern Tkinter viewer/controller for the Arena 03 simulation.

    The combat engine above is intentionally untouched. This class only owns
    application chrome, controls, presentation, logging, and rendering.
    """
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("zpyCombatArena 03 — Combat Viewer")
        ...
        self.seed_var = tk.StringVar(value=str(random.randrange(1_000_000)))
        self.auto_scroll = tk.BooleanVar(value=True)
        self.show_grid = tk.BooleanVar(value=True)
        self.show_names = tk.BooleanVar(value=True)
        self.show_health = tk.BooleanVar(value=True)
        self.camera_shake = tk.BooleanVar(value=True)

        self.status_var = tk.StringVar(value="READY")
        self.time_var = tk.StringVar(value="0.0 s")
        self.seed_display_var = tk.StringVar(value="—")
        self.red_summary_var = tk.StringVar(value="0 alive")
        self.blue_summary_var = tk.StringVar(value="0 alive")
        self.arena_summary_var = tk.StringVar(value="No battle loaded")
        self.selected_name_var = tk.StringVar(value="Select a combatant")
        self.selected_meta_var = tk.StringVar(value="Details will appear here.")
        self.selected_state_var = tk.StringVar(value="")
        self.footer_var = tk.StringVar(
            value="Space: start/pause   Right Arrow: step   Ctrl+N: new fight"
        )
        ...
        self.new_fight(seed=self._seed_from_entry(), start=False)
        self.tick()
    ...
    def configure_styles(self):
        ...
        style.configure("AltCard.TFrame", background=c["card_alt"])
        style.configure("App.TLabel", background=c["window"], foreground=c["text"],
                        font=("Segoe UI", 10))
        ...
        style.configure("Card.TLabel", background=c["card"], foreground=c["text"],
                        font=("Segoe UI", 10))
        style.configure("CardTitle.TLabel", background=c["card"], foreground=c["text"],
                        font=("Segoe UI Semibold", 10))
        style.configure("Header.TLabel", background=c["window"], foreground=c["text"],
                        font=("Segoe UI Semibold", 18))
        style.configure("Subheader.TLabel", background=c["window"], foreground=c["muted"],
                        font=("Segoe UI", 9))
        style.configure("Status.TLabel", background=c["card_alt"], foreground=c["accent"],
                        font=("Segoe UI Semibold", 9), padding=(10, 5))
        style.configure("Footer.TLabel", background=c["panel"], foreground=c["muted"],
                        font=("Segoe UI", 9), padding=(10, 6))

        style.configure("Primary.TButton", background=c["accent"], foreground="#081018",
                        borderwidth=0, padding=(12, 8), font=("Segoe UI Semibold", 9))
        style.map("Primary.TButton",
                  background=[("active", c["accent_hover"]), ("disabled", "#33404d")],
                  foreground=[("disabled", c["faint"])])
        style.configure("Secondary.TButton", background=c["card_alt"], foreground=c["text"],
                        borderwidth=0, padding=(10, 8), font=("Segoe UI", 9))
        style.map("Secondary.TButton", background=[("active", "#2b3743")])
        style.configure("Danger.TButton", background="#40272a", foreground="#ffb0b0",
                        borderwidth=0, padding=(10, 8), font=("Segoe UI", 9))
        style.map("Danger.TButton", background=[("active", "#573034")])
        style.configure("Compact.TButton", background=c["card_alt"], foreground=c["text"],
                        borderwidth=0, padding=(8, 5), font=("Segoe UI", 8))
        style.map("Compact.TButton", background=[("active", "#2b3743")])

        style.configure("TSpinbox", fieldbackground="#11161b", background=c["card_alt"],
                        foreground=c["text"], arrowcolor=c["muted"], borderwidth=1)
        style.configure("TCombobox", fieldbackground="#11161b", background=c["card_alt"],
                        foreground=c["text"], arrowcolor=c["muted"], borderwidth=1)
        style.map("TCombobox", fieldbackground=[("readonly", "#11161b")],
                  foreground=[("readonly", c["text"])])
        style.configure("TEntry", fieldbackground="#11161b", foreground=c["text"],
        ...
        style.configure("TNotebook.Tab", background=c["panel"], foreground=c["muted"],
                        padding=(14, 8), borderwidth=0, font=("Segoe UI", 9))
        style.map("TNotebook.Tab",
                  background=[("selected", c["card"])],
                  foreground=[("selected", c["text"]), ("active", c["text"])])

        style.configure("Treeview", background="#12171c", fieldbackground="#12171c",
                        foreground="#d7dfe7", rowheight=25, borderwidth=0,
                        font=("Segoe UI", 9))
        style.map("Treeview", background=[("selected", c["selection"])],
                  foreground=[("selected", c["text"])])
        style.configure("Treeview.Heading", background="#242d36",
                        foreground=c["muted"], relief="flat", borderwidth=0,
                        font=("Segoe UI Semibold", 8))
        style.map("Treeview.Heading", background=[("active", "#2b3641")])
        style.configure("TPanedwindow", background=c["window"])
        style.configure("Horizontal.TScale", background=c["card"],
                        troughcolor="#11161b")
    ...
    def build_menu(self):
        ...
        file_menu.add_command(label="New Fight", accelerator="Ctrl+N",
                              command=self.new_random_fight)
        file_menu.add_command(label="Rematch", accelerator="Ctrl+R",
                              command=self.rematch)
        file_menu.add_separator()
        file_menu.add_command(label="Save Battle Log…", accelerator="Ctrl+S",
                              command=self.save_log)
        file_menu.add_command(label="Copy Battle Log", command=self.copy_log)
        file_menu.add_separator()
        ...
        sim_menu.add_command(label="Start / Pause", accelerator="Space",
                             command=self.toggle)
        sim_menu.add_command(label="Advance One Tick", accelerator="Right Arrow",
                             command=self.step_once)
        sim_menu.add_separator()
        sim_menu.add_command(label="Restart Same Fight", command=self.rematch)
        menu.add_cascade(label="Simulation", menu=sim_menu)

        view_menu = tk.Menu(menu, tearoff=False)
        view_menu.add_checkbutton(label="Arena grid", variable=self.show_grid,
                                  command=self.render)
        view_menu.add_checkbutton(label="Fighter names", variable=self.show_names,
                                  command=self.render)
        view_menu.add_checkbutton(label="Health bars", variable=self.show_health,
                                  command=self.render)
        view_menu.add_checkbutton(label="Camera shake", variable=self.camera_shake)
    ...
    def build_layout(self):
        ...
        main = ttk.Panedwindow(self.root, orient="horizontal", style="TPanedwindow")
        main.grid(row=1, column=0, sticky="nsew", padx=12, pady=(0, 10))

        arena_card = ttk.Frame(main, style="Card.TFrame", padding=10)
        side_panel = ttk.Frame(main, style="Panel.TFrame", padding=(8, 0, 0, 0))
        main.add(arena_card, weight=3)
        main.add(side_panel, weight=2)

        arena_card.grid_rowconfigure(1, weight=1)
        arena_card.grid_columnconfigure(0, weight=1)

        arena_top = ttk.Frame(arena_card, style="Card.TFrame")
        arena_top.grid(row=0, column=0, sticky="ew", pady=(0, 8))
        arena_top.grid_columnconfigure(1, weight=1)
        ttk.Label(arena_top, text="ARENA", style="CardTitle.TLabel").grid(
            row=0, column=0, sticky="w")
        ttk.Label(arena_top, textvariable=self.arena_summary_var,
                  style="Muted.TLabel").grid(row=0, column=1, sticky="e")

        self.canvas = tk.Canvas(arena_card, width=ARENA_W, height=ARENA_H,
                                bg=self.COLORS["window"], highlightthickness=0,
                                cursor="crosshair")
        ...
        overview = ttk.Frame(self.notebook, style="Panel.TFrame", padding=(2, 8))
        combatants = ttk.Frame(self.notebook, style="Panel.TFrame", padding=(2, 8))
        events = ttk.Frame(self.notebook, style="Panel.TFrame", padding=(2, 8))
        self.notebook.add(overview, text="Overview")
        self.notebook.add(combatants, text="Combatants")
        self.notebook.add(events, text="Event Log")

        self.build_overview_tab(overview)
        self.build_combatants_tab(combatants)
        self.build_events_tab(events)

        footer = ttk.Label(self.root, textvariable=self.footer_var,
                           style="Footer.TLabel", anchor="w")
        footer.grid(row=2, column=0, sticky="ew")

    def build_header(self):
        header = ttk.Frame(self.root, style="App.TFrame", padding=(14, 12, 14, 10))
        header.grid(row=0, column=0, sticky="ew")
        header.grid_columnconfigure(1, weight=1)

        title_box = ttk.Frame(header, style="App.TFrame")
        title_box.grid(row=0, column=0, sticky="w")
        ttk.Label(title_box, text="zpyCombatArena", style="Header.TLabel").pack(anchor="w")
        ttk.Label(title_box, text="Roster-driven tactical combat viewer · Arena 03",
                  style="Subheader.TLabel").pack(anchor="w")

        live = ttk.Frame(header, style="App.TFrame")
        live.grid(row=0, column=1, sticky="e", padx=18)
        ttk.Label(live, textvariable=self.status_var, style="Status.TLabel").grid(
            row=0, column=0, rowspan=2, padx=(0, 14))
        ttk.Label(live, text="TIME", style="Subheader.TLabel").grid(
            row=0, column=1, sticky="e")
        ttk.Label(live, textvariable=self.time_var, style="App.TLabel").grid(
            row=1, column=1, sticky="e")
        ttk.Label(live, text="SEED", style="Subheader.TLabel").grid(
            row=0, column=2, sticky="e", padx=(18, 0))
        ttk.Label(live, textvariable=self.seed_display_var, style="App.TLabel").grid(
            row=1, column=2, sticky="e", padx=(18, 0))

        ttk.Button(header, text="Exit", style="Danger.TButton",
                   command=self.request_exit).grid(row=0, column=2, sticky="e")
    ...
    def build_overview_tab(self, parent):
        ...
        ttk.Label(controls, text="BATTLE CONTROLS", style="CardTitle.TLabel").grid(
            row=0, column=0, columnspan=3, sticky="w", pady=(0, 8))

        self.btn_start = ttk.Button(controls, text="Start", style="Primary.TButton",
                                    command=self.toggle)
        self.btn_start.grid(row=1, column=0, sticky="ew", padx=(0, 4))
        self.btn_step = ttk.Button(controls, text="Step", style="Secondary.TButton",
                                   command=self.step_once)
        self.btn_step.grid(row=1, column=1, sticky="ew", padx=4)
        ttk.Button(controls, text="Rematch", style="Secondary.TButton",
                   command=self.rematch).grid(row=1, column=2, sticky="ew", padx=(4, 0))

        ttk.Label(controls, text="Simulation speed", style="Muted.TLabel").grid(
            row=2, column=0, sticky="w", pady=(12, 2))
        self.speed_label = ttk.Label(controls, text="1.0×", style="Card.TLabel")
        self.speed_label.grid(row=2, column=2, sticky="e", pady=(12, 2))
        speed_scale = ttk.Scale(controls, from_=0.5, to=8.0, variable=self.speed,
                                command=self.on_speed_change,
                                style="Horizontal.TScale")
        speed_scale.grid(row=3, column=0, columnspan=3, sticky="ew")

        setup = ttk.Frame(parent, style="Card.TFrame", padding=10)
        setup.grid(row=1, column=0, sticky="ew", pady=(0, 8))
        setup.grid_columnconfigure(1, weight=1)
        setup.grid_columnconfigure(3, weight=1)
        ttk.Label(setup, text="NEXT FIGHT", style="CardTitle.TLabel").grid(
            row=0, column=0, columnspan=4, sticky="w", pady=(0, 7))

        ttk.Label(setup, text="Red", style="Muted.TLabel").grid(
            row=1, column=0, sticky="w", pady=2)
        ttk.Spinbox(setup, from_=1, to=5, width=4, textvariable=self.size_a,
                    state="readonly").grid(row=1, column=1, sticky="w", pady=2)
        ttk.Label(setup, text="Blue", style="Muted.TLabel").grid(
            row=1, column=2, sticky="e", padx=(12, 5), pady=2)
        ttk.Spinbox(setup, from_=1, to=5, width=4, textvariable=self.size_b,
                    state="readonly").grid(row=1, column=3, sticky="w", pady=2)

        ttk.Label(setup, text="Weapons", style="Muted.TLabel").grid(
            row=2, column=0, sticky="w", pady=2)
        ttk.Combobox(setup, textvariable=self.weap_mode,
                     values=("Best fit", "Random"), state="readonly",
                     width=14).grid(row=2, column=1, columnspan=3, sticky="ew", pady=2)
        ttk.Label(setup, text="Seed", style="Muted.TLabel").grid(
            row=3, column=0, sticky="w", pady=2)
        seed_entry = ttk.Entry(setup, textvariable=self.seed_var, width=14)
        seed_entry.grid(row=3, column=1, columnspan=2, sticky="ew", pady=2)
        ttk.Button(setup, text="Randomize", style="Compact.TButton",
                   command=self.randomize_seed).grid(row=3, column=3, sticky="e", padx=(6, 0))
        ttk.Button(setup, text="Create New Fight", style="Primary.TButton",
                   command=self.new_configured_fight).grid(
        ...
        ttk.Label(summary, text="TEAM STATUS", style="CardTitle.TLabel").grid(
            row=0, column=0, columnspan=2, sticky="w", pady=(0, 8))
        red_box = tk.Frame(summary, bg="#2b2022", padx=10, pady=9)
        red_box.grid(row=1, column=0, sticky="ew", padx=(0, 4))
        tk.Label(red_box, text="RED", bg="#2b2022", fg=self.COLORS["red"],
                 font=("Segoe UI Semibold", 9)).pack(anchor="w")
        tk.Label(red_box, textvariable=self.red_summary_var, bg="#2b2022",
                 fg=self.COLORS["text"], font=("Segoe UI", 10)).pack(anchor="w")
        blue_box = tk.Frame(summary, bg="#1e2632", padx=10, pady=9)
        blue_box.grid(row=1, column=1, sticky="ew", padx=(4, 0))
        tk.Label(blue_box, text="BLUE", bg="#1e2632", fg=self.COLORS["blue"],
                 font=("Segoe UI Semibold", 9)).pack(anchor="w")
        tk.Label(blue_box, textvariable=self.blue_summary_var, bg="#1e2632",
                 fg=self.COLORS["text"], font=("Segoe UI", 10)).pack(anchor="w")

        selected = ttk.Frame(parent, style="Card.TFrame", padding=9)
        selected.grid(row=3, column=0, sticky="nsew")
        selected.grid_columnconfigure(0, weight=1)
        ttk.Label(selected, text="SELECTED COMBATANT", style="CardTitle.TLabel").grid(
            row=0, column=0, sticky="w")
        ttk.Label(selected, textvariable=self.selected_name_var,
                  style="Card.TLabel", font=("Segoe UI Semibold", 12)).grid(
            row=1, column=0, sticky="w", pady=(10, 2))
        ttk.Label(selected, textvariable=self.selected_meta_var,
                  style="Muted.TLabel", wraplength=360, justify="left").grid(
            row=2, column=0, sticky="w")
        ttk.Label(selected, textvariable=self.selected_state_var,
                  style="Muted.TLabel", wraplength=360, justify="left").grid(
            row=3, column=0, sticky="w", pady=(7, 0))

    def build_combatants_tab(self, parent):
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=1)
        cols = ("team", "name", "weapon", "skill", "hp", "state", "dps", "dmg")
        self.tree = ttk.Treeview(parent, columns=cols, show="headings", selectmode="browse")
        headings = {
            "team": "TEAM", "name": "COMBATANT", "weapon": "WEAPON",
            "skill": "SKL", "hp": "HP", "state": "STATE",
            "dps": "DPS", "dmg": "DMG",
        }
        widths = {"team": 50, "name": 112, "weapon": 95, "skill": 40,
                  "hp": 70, "state": 70, "dps": 45, "dmg": 50}
        anchors = {"name": "w", "weapon": "w", "state": "w"}
        for col in cols:
            self.tree.heading(col, text=headings[col], command=lambda c=col: self.sort_tree(c))
            self.tree.column(col, width=widths[col], minwidth=38,
                             anchor=anchors.get(col, "center"), stretch=col in ("name", "weapon"))
        self.tree.tag_configure("t0", foreground=self.COLORS["red"])
        self.tree.tag_configure("t1", foreground=self.COLORS["blue"])
        self.tree.tag_configure("dead", foreground="#5f6973")
    ...
    def build_events_tab(self, parent):
        ...
        ttk.Button(toolbar, text="Copy", style="Compact.TButton",
                   command=self.copy_log).pack(side="left")
        ttk.Button(toolbar, text="Save…", style="Compact.TButton",
                   command=self.save_log).pack(side="left", padx=5)
        ttk.Checkbutton(toolbar, text="Auto-scroll", variable=self.auto_scroll).pack(side="right")

        log_frame = ttk.Frame(parent, style="Card.TFrame")
        log_frame.grid(row=1, column=0, sticky="nsew")
        log_frame.grid_rowconfigure(0, weight=1)
        log_frame.grid_columnconfigure(0, weight=1)
        self.log_text = tk.Text(log_frame, bg=self.COLORS["log"], fg="#cfd8e1",
                                relief="flat", borderwidth=0, state="disabled",
                                font=("Cascadia Mono", 9), wrap="word",
                                padx=10, pady=9, insertbackground=self.COLORS["text"])
        scrollbar = ttk.Scrollbar(log_frame, orient="vertical", command=self.log_text.yview)
        self.log_text.configure(yscrollcommand=scrollbar.set)
        self.log_text.grid(row=0, column=0, sticky="nsew")
        scrollbar.grid(row=0, column=1, sticky="ns")
        self.log_text.tag_configure("time", foreground="#66727d")
        self.log_text.tag_configure("normal", foreground="#cfd8e1")
        self.log_text.tag_configure("crit", foreground="#ffd966")
        self.log_text.tag_configure("block", foreground="#91b8dc")
        self.log_text.tag_configure("danger", foreground="#ff8585")
        self.log_text.tag_configure("effect", foreground="#b8da9a")
        self.log_text.tag_configure("boom", foreground="#ffad66")
        self.log_text.tag_configure("result", foreground="#75d98b",
                                    font=("Cascadia Mono", 9, "bold"))

    def bind_shortcuts(self):
        self.root.bind_all("<space>", lambda _e: self.toggle())
        self.root.bind_all("<Right>", lambda _e: self.step_once())
        self.root.bind_all("<Escape>", lambda _e: self.pause())
        self.root.bind_all("<Control-n>", lambda _e: self.new_random_fight())
        self.root.bind_all("<Control-r>", lambda _e: self.rematch())
        self.root.bind_all("<Control-s>", lambda _e: self.save_log())
        self.root.bind_all("<Control-q>", lambda _e: self.request_exit())

    # ------------------------------------------------------------ controls

    def _seed_from_entry(self):
    ...
    def randomize_seed(self):
        self.seed_var.set(str(random.randrange(1_000_000)))

    def new_random_fight(self):
        self.randomize_seed()
        self.new_configured_fight()

    def new_configured_fight(self):
        try:
            seed = self._seed_from_entry()
        except ValueError as exc:
            messagebox.showerror("Invalid seed", str(exc), parent=self.root)
            return
        self.new_fight(seed=seed, start=False)

    def rematch(self):
        if self.sim is None:
            return
        self.seed_var.set(str(self.sim.seed))
        self.new_fight(seed=self.sim.seed, start=False)

    def new_fight(self, seed, start=False):
        roll = random.Random(seed ^ 0xC0FFEE)
        na, nb = self.size_a.get(), self.size_b.get()
        if na + nb > len(self.roster):
            messagebox.showerror(
                "Not enough characters",
                f"This setup needs {na + nb} characters, but the roster contains "
                f"only {len(self.roster)}.",
                parent=self.root,
            )
            return
        ...
        self.rebuild_table()
        self.update_ui(force_render=True)
        self.set_running(self.running)
        if self.sim.fighters:
            self.select_fighter(self.sim.fighters[0])

    def set_running(self, running):
        if self.sim is not None and self.sim.winner is not None:
            running = False
        self.running = running
        self.btn_start.configure(text="Pause" if running else "Start")
        self.btn_step.state(["disabled"] if running else ["!disabled"])
        self.status_var.set("RUNNING" if running else
                            ("COMPLETE" if self.sim and self.sim.winner else "PAUSED"))
    ...
    def step_once(self):
        if self.sim is None or self.running or self.sim.winner is not None:
            return
        self.sim.step()
        self.update_ui(force_render=True)
        if self.sim.winner is not None:
    ...
    def on_speed_change(self, _value=None):
        value = round(float(self.speed.get()) * 2) / 2
        self.speed.set(value)
        self.speed_label.configure(text=f"{value:.1f}×")
    ...
    def tick(self):
        ...
        if self.running and self.sim is not None:
            ...
            while self.accum >= 1.0 and self.sim.winner is None:
                self.sim.step()
                self.accum -= 1.0
            self.update_ui(force_render=True)
            if self.sim.winner is not None:
                self.set_running(False)
        self.after_id = self.root.after(33, self.tick)

    def update_ui(self, force_render=False):
        self.flush_log()
        self.update_table()
        self.update_summary()
    ...
    def update_summary(self):
        if self.sim is None:
            return
        self.time_var.set(f"{self.sim.t:0.1f} s")
        red = [f for f in self.sim.fighters if f.team == 0]
        blue = [f for f in self.sim.fighters if f.team == 1]

        def team_text(group):
            alive = [f for f in group if f.alive]
            hp = sum(max(f.hp, 0) for f in alive)
            max_hp = sum(f.max_hp for f in group)
            return f"{len(alive)}/{len(group)} alive · {hp:.0f}/{max_hp} HP"

        self.red_summary_var.set(team_text(red))
        self.blue_summary_var.set(team_text(blue))
        projectiles = len(self.sim.projectiles) + len(self.sim.spawn_queue)
        self.arena_summary_var.set(
            f"{sum(f.alive for f in self.sim.fighters)} standing  ·  "
            f"{projectiles} projectiles  ·  {len(self.sim.hazards)} hazards"
        )
        if self.sim.winner is not None:
            self.status_var.set("COMPLETE")
            if self.sim.winner == "draw":
                self.footer_var.set(f"Draw after {self.sim.t:.1f} seconds · seed {self.sim.seed}")
            else:
                self.footer_var.set(
                    f"{self.sim.winner} won after {self.sim.t:.1f} seconds · seed {self.sim.seed}"
                )
        elif self.running:
            self.footer_var.set("Simulation running · Space pauses · Esc pauses immediately")
        else:
            self.footer_var.set("Paused · Space starts · Right Arrow advances one simulation tick")
    ...
    def rebuild_table(self):
        ...
        self.rows = {}
        for f in self.sim.stable_order():
    ...
    def update_table(self):
        ...
        for f in self.sim.stable_order():
            iid = self.rows.get(id(f))
            if not iid:
                continue
            state = "DOWN" if f.down > 0 else ("FLEE" if f.flee > 0 else f.state)
            effects = "" if not f.effects else " +" + ",".join(e.kind[0].upper() for e in f.effects)
            self.tree.item(iid, values=(
                "RED" if f.team == 0 else "BLUE",
                f.name,
                f.weapon.name,
                f"{f.skill:.0f}",
    ...
    def sort_tree(self, column):
        ...
        numeric = column in {"skill", "dps", "dmg"}
    ...
    def on_tree_select(self, _event=None):
        ...
        if not selected or self.sim is None:
            return
        iid = selected[0]
        for f in self.sim.fighters:
            if self.rows.get(id(f)) == iid:
                self.select_fighter(f)
                break

    def select_fighter(self, fighter):
        self.selected_fighter = fighter
        iid = self.rows.get(id(fighter))
        if iid:
    ...
    def update_selected_details(self):
        ...
        if f is None:
            self.selected_name_var.set("Select a combatant")
            self.selected_meta_var.set("Details will appear here.")
            self.selected_state_var.set("")
            return
        team = "Red team" if f.team == 0 else "Blue team"
        self.selected_name_var.set(f.name)
        self.selected_meta_var.set(
            f"{team} · {f.weapon.name} ({f.weapon.wtype}) · skill {f.skill:.0f}\n"
            f"HP {max(f.hp, 0):.0f}/{f.max_hp} · damage {f.dmg_dealt:.0f} · "
            f"move {f.move_speed:.0f}"
        )
        target = f.target.name if f.target is not None and f.target.alive else "none"
        effects = ", ".join(e.kind for e in f.effects) or "none"
        state = "dead" if not f.alive else (
            "knocked down" if f.down > 0 else "fleeing" if f.flee > 0 else f.state.lower()
        )
        self.selected_state_var.set(
            f"State: {state} · Target: {target}\n"
            f"Effects: {effects} · Dodge {f.dodge_stat:.0f} · Block {f.block_stat:.0f}"
        )
    ...
    def _log_tag(self, text):
        lower = text.lower()
        if "wins in" in lower or "draw" in lower:
            return "result"
        if "crit" in lower:
            return "crit"
        if "blocks" in lower or "dodges" in lower or "thunk" in lower:
            return "block"
        if "is down" in lower or "friendly fire" in lower:
            return "danger"
        if "bleeding" in lower or "poison" in lower or "catches fire" in lower:
            return "effect"
        if "boom" in lower or "blown off" in lower:
            return "boom"
        return "normal"
    ...
    def flush_log(self, clear=False):
        ...
        for t, text in self.sim.events[self.logged:]:
            self.log_text.insert("end", f"[{t:6.2f}] ", "time")
    ...
    def log_as_text(self):
        if self.sim is None:
            return ""
        lines = [
            "zpyCombatArena 03 battle log",
            f"Seed: {self.sim.seed}",
            f"Result: {self.sim.winner or 'in progress'}",
            f"Time: {self.sim.t:.2f} seconds",
            "",
        ]
        lines.extend(f"[{t:6.2f}] {text}" for t, text in self.sim.events)
        return "\n".join(lines) + "\n"

    def copy_log(self):
        text = self.log_as_text()
        if not text:
            return
        self.root.clipboard_clear()
        self.root.clipboard_append(text)
        self.footer_var.set("Battle log copied to the clipboard")
    ...
    def save_log(self):
        ...
        path = filedialog.asksaveasfilename(
            parent=self.root,
            title="Save battle log",
            defaultextension=".txt",
            initialfile=f"arena_seed_{self.sim.seed}.txt",
        ...
        try:
            with open(path, "w", encoding="utf-8") as fh:
                fh.write(self.log_as_text())
        except OSError as exc:
            messagebox.showerror("Could not save log", str(exc), parent=self.root)
            return
        self.footer_var.set(f"Battle log saved to {os.path.basename(path)}")
    ...
    def show_shortcuts(self):
        messagebox.showinfo(
            ...
            "Right Arrow Advance one simulation tick\n"
            "Escape      Pause\n"
            "Ctrl+N      Create a new random fight\n"
            "Ctrl+R      Rematch the current seed\n"
            "Ctrl+S      Save the battle log\n"
            "Ctrl+Q      Exit",
            parent=self.root,
        )
    ...
    def show_about(self):
        messagebox.showinfo(
            "About zpyCombatArena",
            "zpyCombatArena 03\n\n"
            "A deterministic, roster-driven top-down team combat simulation.\n"
            "This interface controls and visualizes the existing Arena 03 engine.",
            parent=self.root,
        )
    ...
    def request_exit(self):
        ...
        if self.running:
            close = messagebox.askyesno(
                "Exit zpyCombatArena?",
                "A battle is currently running. Exit the application?",
                icon="warning",
                parent=self.root,
            )
            if not close:
    ...
    def on_canvas_click(self, event):
        """Select the fighter nearest a click in the scaled arena view."""
        if self.sim is None:
            return
        scale = getattr(self, "arena_scale", 1.0)
        ox = getattr(self, "arena_offset_x", 0.0)
        oy = getattr(self, "arena_offset_y", 0.0)
        if scale <= 0:
            return
        ax = (event.x - ox) / scale
        ay = (event.y - oy) / scale
        fighter = min(self.sim.fighters,
                      key=lambda f: math.hypot(f.x - ax, f.y - ay),
                      default=None)
        if fighter is not None and math.hypot(fighter.x - ax, fighter.y - ay) <= 24:
            self.select_fighter(fighter)

    def render(self):
        if self.sim is None or not hasattr(self, "canvas"):
            return
        c = self.canvas
        c.delete("all")
        arena_bg = self.COLORS["arena"]
        c.create_rectangle(0, 0, ARENA_W, ARENA_H, fill=arena_bg, outline="")

        if self.show_grid.get():
            for gx in range(0, ARENA_W + 1, 40):
                c.create_line(gx, 0, gx, ARENA_H, fill=self.COLORS["arena_grid"])
            for gy in range(0, ARENA_H + 1, 40):
                c.create_line(0, gy, ARENA_W, gy, fill=self.COLORS["arena_grid"])
        c.create_rectangle(WALL_PAD - 14, WALL_PAD - 14,
                           ARENA_W - WALL_PAD + 14, ARENA_H - WALL_PAD + 14,
                           outline=self.COLORS["arena_border"], width=3)
        ...
        # Selected target line is a viewer aid only.
        sf = self.selected_fighter
        if sf is not None and sf.alive and sf.target is not None and sf.target.alive:
            c.create_line(sf.x, sf.y, sf.target.x, sf.target.y,
                          fill="#7f96aa", dash=(4, 5), width=1)
        ...
        c.create_rectangle(7, 7, 183, 31, fill="#141a20", outline="#303b46")
        c.create_text(16, 19, anchor="w", fill="#94a2af", font=("Cascadia Mono", 9),
                      text=f"TIME {self.sim.t:6.1f}   SEED {self.sim.seed}")

        if self.sim.winner is not None:
            c.create_rectangle(130, ARENA_H / 2 - 62, ARENA_W - 130,
                               ARENA_H / 2 + 62, fill="#11171d",
                               outline="#536170", width=2)
            msg = "DRAW" if self.sim.winner == "draw" else self.sim.winner.upper() + " WINS"
            color = self.COLORS["warning"] if self.sim.winner == "draw" else (
                self.COLORS["red"] if self.sim.winner == TEAM_NAMES[0]
                else self.COLORS["blue"])
            c.create_text(ARENA_W / 2, ARENA_H / 2 - 15, text=msg,
                          fill=color, font=("Segoe UI Semibold", 25))
            c.create_text(ARENA_W / 2, ARENA_H / 2 + 22,
                          text="Rematch repeats the seed · New Fight creates new teams",
                          fill=self.COLORS["muted"], font=("Segoe UI", 9))

        self.fit_arena_to_canvas()

    def fit_arena_to_canvas(self):
        width = max(self.canvas.winfo_width(), 1)
        height = max(self.canvas.winfo_height(), 1)
        scale = min(width / ARENA_W, height / ARENA_H)
        offset_x = (width - ARENA_W * scale) / 2
        offset_y = (height - ARENA_H * scale) / 2
        self.arena_scale = scale
        self.arena_offset_x = offset_x
        self.arena_offset_y = offset_y
        self.canvas.scale("all", 0, 0, scale, scale)
        self.canvas.move("all", offset_x, offset_y)
        if self.camera_shake.get() and self.sim.shake > 0.4:
            self.canvas.move("all", random.uniform(-1, 1) * self.sim.shake * scale,
                             random.uniform(-1, 1) * self.sim.shake * scale)

    def draw_fighter(self, f):
        c = self.canvas
        r = f.radius
        selected = f is self.selected_fighter

        if selected:
            c.create_oval(f.x - r - 7, f.y - r - 7, f.x + r + 7, f.y + r + 7,
                          outline="#eef3f8", width=2, dash=(3, 2))

        if not f.alive:
            da = getattr(f, "drop_ang", 0.8)
            wd = f.weapon
            if wd.wtype == "explosive":
        ...
        if f.down > 0:
            c.create_oval(f.x - r - 3, f.y - r * 0.55, f.x + r + 3,
                          f.y + r * 0.55, fill=f.color, outline="#0b0f13", width=2)
            self.draw_bars(f)
            return
        ...
        if f.state == SWING:
            ...
            for k in (0.75, 0.5, 0.25):
                ...
                c.create_line(f.x + math.cos(ta) * r,
                              ...
                              fill=mix(wd.color, self.COLORS["arena"], 1 - k * 0.8),
                              width=max(1, wd.width - 2))
        elif f.state == WINDUP:
            denom = wd.windup * f.atk_speed
            frac = 1 - f.t_state / denom if denom > 0 else 1
    ...
    def draw_bars(self, f):
        ...
        col = self.COLORS["good"] if frac > 0.5 else (
            self.COLORS["warning"] if frac > 0.25 else self.COLORS["danger"])
        if self.show_health.get():
            c.create_rectangle(f.x - bw / 2, f.y - r - 12, f.x + bw / 2,
                               f.y - r - 6, fill="#0d1115", outline="#303840")
            c.create_rectangle(f.x - bw / 2 + 1, f.y - r - 11,
                               f.x - bw / 2 + 1 + (bw - 2) * frac,
                               f.y - r - 7, fill=col, outline="")
        for i, e in enumerate(f.effects):
            ex = f.x - bw / 2 + 4 + i * 8
            c.create_oval(ex - 3, f.y - r - 19, ex + 3, f.y - r - 13,
                          fill=EFFECT_COLORS[e.kind], outline="#11161b")
        if self.show_names.get():
            c.create_text(f.x, f.y + r + 11, text=f.name, fill="#d5dde5",
                          font=("Segoe UI", 8, "bold"))
