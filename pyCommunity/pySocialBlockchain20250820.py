...
# Config & Constants
# =========================
SIM_CYCLES = 10            # number of cycles (days)
RNG_SEED = 12345           # set None for full randomness
VERBOSE_ACTIONS = True     # print every action in detail
VERBOSE_TICK = True        # print per-cycle header and footer summaries
...
class World:
    ...
    def _init_population(self):
        ...
        for prof in ALL_PROFESSIONS:
            for _ in range(2):
                ...
                self.toons.append(t)
                # --- NEW: seed starting resources for processors/knowledge roles, + Joules for all ---
                self._seed_starting_items(t)
                counter += 1

    # Mapping of profession → list of input items they need to perform their job
    STARTER_INPUTS_BY_PROF = {
        ...
        # Merchant: no required inputs (they can buy), left empty to honor 3-slot limit when adding Joules
    }
    ...
    def _seed_starting_items(self, t):
        ...
        # Add each required input (up to inventory-type capacity)
        for it in inputs:
            t.add_item(it, 5)
        # Everyone gets 5 Joules (added after inputs so it doesn't block them)
        t.add_item(JOULES, 5)
    ...
    def award_block(self, toon, cycle):
        # used if we ever want to tally globally, but toon.try_mint_block prints already
        self.blocks_total += 1
        self.block_log.append(f"c{cycle:03d}_{toon.name}_{toon.profession}")

    def merchant_buy_from_producer(self, merchant, item):
        """
        Merchant tries to buy one unit of 'item' from any producer who has it,
        paying 1 credit to seller, then stores it.
        """
    ...
    def merchant_deliver(self, merchant, needy):
        """
        Merchant scans needy.needs_resource and tries to deliver 1 unit for 1 credit.
        Will attempt to source by buying first if merchant lacks stock.
        """
        for needed in list(needy.needs_resource):
            if merchant.inv.qty(needed) == 0:
                # Try to buy from producers first
    ...
    def do_self_focused(self, t):
        # pick context-appropriate self action
        choice = None
        ...
        if choice == "eat":
            ...
            # Openness influences chance to succeed at a training tick
            if t.attr("Openness") >= 10 or self.rnd.random() < 0.3:
                cand = self.rnd.choice(list(t.attributes.keys()))
                if t.attributes[cand] < 20:
                    t.attributes[cand] += 1
                    if VERBOSE_ACTIONS:
                        print(f"      [Train] {t.name} improved {cand} → {t.attributes[cand]}")
    ...
    def do_professional(self, t):
        ...
        if prof == "Echokeeper":
            # Supportive professional: soothe a random neighbor's mood
        ...
        if prof == "Merchant":
            # Try to satisfy one needy toon
            # Merchants may trade with 'done' toons; ignore done flag
            needy_pool = [x for x in self.toons if x.needs_resource]
            # prefer closest by relationship or random
        ...
        for inputs, outputs in recs:
            ...
            if can:
                ...
                for ot, n in outputs.items():
                    if VERBOSE_ACTIONS:
                        print(f"  [Work]  {t.name} ({prof}) crafts {' + '.join([f'{n} {ot}' for ot,n in outputs.items()])}")
                    t.add_item(ot, n)
                return

        # If we got here, we lacked inputs; flag them
        if recs:
            needed_any = []
    ...
    def do_interactive(self, t):
        # Choose target: sample up to 10 not-done; pick strongest +/- relationship to promote repeated ties
        ...
        # rank by extreme relationship (max abs)
        def rel_to(x):
            return t.relations.get(x.name, 0)

        target = max(candidates, key=lambda x: abs(rel_to(x)))

        # Choose an interaction; for now only "Compliment"
        label = "Compliment"
        ...
        # Outcome roll with target preference
        # keep trying until target personality accepts (to avoid dead interactions)
        outcome_label = None
        for _ in range(5):
        ...
        # Attribute drift (gated by target openness)
        drift = eff.get("drift")
        if drift:
            attr, d = drift
    ...
    def run_cycle(self, c):
        ...
        for t in order:
            if t.life <= 0:
                # keep them in limbo (could later add revival logic)
                if VERBOSE_ACTIONS:
            ...
            # After acting, try mint block(s)
            before_blocks = t.blocks
            t.try_mint_block(c)
            # track world block tally (optional)
            if t.blocks > before_blocks:
                self.blocks_total += (t.blocks - before_blocks)
                # also store log lines (already printed)
                for _ in range(t.blocks - before_blocks):
        ...
        if VERBOSE_TICK:
            ...
            print(f"Destinyblocks total so far: {self.blocks_total}")
            print("Top credits:")
            for x in top:
