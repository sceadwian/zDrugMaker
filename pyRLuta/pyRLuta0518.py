...
def record_to_leaderboard(player, won_fight):
    ...
    if not won_fight and player.save_id is not None:
        ...
        if SAVED_CHARS_DATA: # Ensure SAVED_CHARS_DATA is not empty before accessing keys
             # If SAVED_CHARS_DATA can be empty, provide default fieldnames or handle empty case
            if SAVED_CHARS_DATA: # Re-check after potential modification
                 save_fieldnames = list(SAVED_CHARS_DATA[0].keys())
            # else, use the default save_fieldnames if it becomes empty
        elif not os.path.exists(CSV_FILES["saved"]) or os.path.getsize(CSV_FILES["saved"]) == 0 :
             # If file is empty or doesn't exist, write header with default fieldnames
             pass # Header will be written with default save_fieldnames
        else: # File exists but SAVED_CHARS_DATA is empty (all chars deleted), effectively means empty file
            # We still need to write the header for an empty file.
            pass
        ...
        try:
            with open(CSV_FILES["saved"], 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writeheader()
                writer.writerows(SAVED_CHARS_DATA) # This is fine even if SAVED_CHARS_DATA is empty
...
def combat_loop(player, opponent):
    ...
    opponent.combat_start_time = time.time() # Opponent also needs this for their DPS if we track it
    player.total_damage_dealt_session = 0
    player.total_damage_taken_session = 0
    opponent.total_damage_dealt_session = 0 # Initialize for opponent as well
    ...
    display_hud(player, opponent) 
    print("\n--- COMBAT END ---")
    for entry in COMBAT_LOG: print(entry) # Print full log once more, now with colors
...
if __name__ == "__main__":
    if os.name == 'nt':
        os.system('') # Initializes virtual terminal processing for ANSI codes on Windows
    if os.name == 'posix':
        _init_tty_for_input_non_blocking()
