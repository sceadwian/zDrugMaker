...
def record_to_leaderboard(player, won_fight):
    ...
    # Recalculate current attributes (including gear bonuses and active effects)
    ...
    # Write out the recalculated, current attributes instead of the base ones
    for attr in ATTRIBUTES:
        entry[attr] = player.current_attributes.get(attr, 0)

    fieldnames = (
        ['timestamp', 'toon_name', 'level', 'kills', 'damage_done', 'damage_received']
    ...
    # If the player lost and has a save slot, remove them from saved chars
    if not won_fight and player.save_id is not None:
        ...
        # Prepare fieldnames for rewriting the saved file
        save_fieldnames = (
