...
class Entity:
    ...
    def apply_effect(self, action_obj, source_entity_is_self): # True if source is applying to self
        # Determine if this entity is the one performing the action or the target
        # Self Buff/Debuff
        ...
        if not (applies_to_self or applies_as_enemy_debuff):
            return "" # Action not meant for this entity in this context

        if len(self.active_effects) >= MAX_ACTIVE_EFFECTS:
            # Simple: just fail if max effects. Could add logic to overwrite weaker/shorter effects.
        ...
        # Process self buffs/debuffs if this entity is the source
        if source_entity_is_self:
            if action_obj.self_buff_target_stat:
                ...
                if action_obj.self_buff_target_stat == "HP":
                    if action_obj.timing == 'tick': effect_mods["hp_change_tick"] = (action_obj.base_val / action_obj.duration) if action_obj.duration > 0 else action_obj.base_val
                        ...
                        log_msg_part += f"{self.name} heals {heal_amount:.0f} HP. "
                elif action_obj.self_buff_target_stat in ATTRIBUTES + ["AtkPw", "MgcPw", "AtkSp", "Block", "Dodge", "Armor", "MgcRs", "Crits"]: # Check if stat is valid
            ...
            if action_obj.self_debuff_target_stat:
                is_buff = False # Even if a buff was also applied, a debuff makes it "mixed" or "debuff" overall
                if action_obj.self_debuff_target_stat == "HP":
                    if action_obj.timing == 'tick': effect_mods["hp_change_tick"] = -(action_obj.base_val / action_obj.duration) if action_obj.duration > 0 else -action_obj.base_val
                        ...
                        log_msg_part += f"{self.name} takes {damage_amount:.0f} self-damage. "
        ...
        # Process enemy debuffs if this entity is the target
        if not source_entity_is_self and action_obj.enemy_debuff_target_stat:
            is_buff = False
            if action_obj.enemy_debuff_target_stat == "HP": # DoT
                if action_obj.timing == 'tick': effect_mods["hp_change_tick"] = -(action_obj.base_val / action_obj.duration) if action_obj.duration > 0 else -action_obj.base_val
                elif action_obj.timing == 'end': effect_mods["hp_change_on_end"] = -action_obj.base_val
                # Instant HP debuff on enemy usually comes from DMG, not DEBUFF:HP.
    ...
    def update_active_effects(self, refresh_interval):
        ...
        for i in range(len(self.active_effects) - 1, -1, -1):
            ...
            if effect["mods"].get("hp_change_tick", 0) != 0:
                ...
                actual_hp_change = 0
                if hp_change_this_tick > 0: # Healing
                    actual_hp_change = min(hp_change_this_tick, self.max_hp - self.current_hp)
                    self.current_hp += actual_hp_change
                    if actual_hp_change > 0.01 : log_updates.append(f"{self.name} heals {actual_hp_change:.1f} HP from {effect['name']}.")
                else: # Damage
                    actual_hp_change = min(abs(hp_change_this_tick), self.current_hp)
                    self.current_hp -= actual_hp_change
                    self.total_damage_taken_session += actual_hp_change # track DoT as damage taken
                    if actual_hp_change > 0.01 : log_updates.append(f"{self.name} takes {actual_hp_change:.1f} damage from {effect['name']}.")
            ...
            if effect["duration_left"] <= 0:
                if effect["timing"] == 'end' and effect["mods"].get("hp_change_on_end", 0) != 0:
                    ...
                    if hp_change > 0:
                         ...
                         self.current_hp += heal_amount
                         log_updates.append(f"{self.name} heals {heal_amount:.0f} HP as {effect['name']} ends.")
                    else:
                         damage_amount = min(abs(hp_change), self.current_hp)
                         self.current_hp -= damage_amount
                         self.total_damage_taken_session += damage_amount
                         log_updates.append(f"{self.name} takes {damage_amount:.0f} damage as {effect['name']} ends.")
                    self.current_hp = max(0, min(self.max_hp, self.current_hp))
...
def select_character():
    ...
    # Temporarily restore terminal for input choice if on POSIX
    ...
    is_saved = False
    player_name_for_entity = "Player" # Default

    if source_choice_idx == 0: # Default
        if not TOONS_DATA:
...
def select_opponent():
    ...
    npc_display_list = []
    npc_id_map = [] # To map display index back to actual NPC ID
    ...
    # NPCS_DATA keys might be strings from CSV if not converted, ensure lookup matches
    npc_data_to_load = NPCS_DATA.get(selected_npc_id_from_map) 
    if not npc_data_to_load: # Fallback if ID was int but stored as str or vice-versa
...
def display_hud(player, opponent):
    ...
    elif p_action_id: # Action ID exists but not in ALL_ACTIONS (data error)
    ...
    dps = (player.total_damage_dealt_session / elapsed_time) if elapsed_time > 0.1 else 0 # Avoid div by zero
    print(f"DPS: {dps:.1f}")
    ...
    o_action_id = opponent.npc_data_source.get(f'act{(opponent.action_sequence_index % 9) + 1}') # next action
    ...
    # o_dps can be calculated similarly if needed
...
def record_to_leaderboard(player, won_fight):
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
    entry = {
        'timestamp': timestamp,
        'toon_name': player.name,
        'level': getattr(player, 'level', 1), # Player's level if exists
    ...
    for attr in ATTRIBUTES:
        entry[attr] = player.base_attributes.get(attr,0)
    
    fieldnames = ['timestamp', 'toon_name', 'level', 'kills', 'damage_done', 'damage_received'] + \
                 [f'item{i}' for i in range(1,9)] + ATTRIBUTES
    ...
    if not won_fight and player.save_id is not None:
        global SAVED_CHARS_DATA
        new_saved_data = [char_d for char_d in SAVED_CHARS_DATA if str(char_d.get('SaveID')) != str(player.save_id)]
        
        SAVED_CHARS_DATA = new_saved_data # Update in-memory list

        # Define fieldnames based on one of the entries or a default structure
        save_fieldnames = ['SaveID', 'Name'] + ATTRIBUTES + [f'Slot{i}' for i in range(1,9)] + ['XP']
        if SAVED_CHARS_DATA: # If list not empty, use keys from first entry
...
def combat_loop(player, opponent):
    ...
    while not game_over:
        ...
        if delta_time <= 0: delta_time = REFRESH_RATE # Ensure positive delta time
    ...
    # Post Combat
    if os.name == 'posix': _restore_tty_settings_non_blocking() # Restore before any input()
    display_hud(player, opponent) # Final display
    print("\n--- COMBAT END ---")
    for entry in COMBAT_LOG: print(entry)

    if player_won:
        if opponent.text_death: add_to_combat_log(opponent.text_death) # Add to log
        ...
        # Save character if it was a default one or if it's an existing saved char
        if player.is_player: # Always save player on win if they are a player entity
    ...
    record_to_leaderboard(player, player_won)
    
    input("\nPress Enter to continue...")
    if os.name == 'posix': _init_tty_for_input_non_blocking() # Re-enable for next game loop/keypresses
...
if __name__ == "__main__":
    if os.name == 'posix':
        _init_tty_for_input_non_blocking()
        atexit.register(_restore_tty_settings_non_blocking) # Ensure restoration on exit
    ...
    try:
        ...
        if os.name == 'posix':
            _restore_tty_settings_non_blocking() # Final attempt to restore