...
def render_race_progress(cars, track_length, sim_time, display_width=100):
    ...
    print(f"{Colors.WHITE}{'Race time: ' + format(sim_time, '.1f') + 's':^{display_width}}{Colors.RESET}")
    print(f"{Colors.CYAN}{'='*display_width}{Colors.RESET}\n")

    for i, car in enumerate(sorted_cars, 1):
        progress = int((car.distance % track_length) / track_length * (display_width - 20))
...
def simulate_race(track_sequence, cars, num_laps, time_step=0.1, quiet=False):
    ...
    while any(not car.finished for car in cars):
        ...
        if not quiet:
            render_race_progress(cars, track_length, sim_time)
...
def main():
    while True:
        ...
        print(f"{Colors.YELLOW}{'pyRacing v04':^50}{Colors.RESET}")
        print(f"{Colors.CYAN}{'='*50}{Colors.RESET}")
        print("  1. Run a race")
        print("  2. Analyze a track & generate sequence file")
        print("  3. View a track layout")
        print("  4. Generate random drivers")
        print("  5. Car physics graph tool")
        print("  q. Quit")
