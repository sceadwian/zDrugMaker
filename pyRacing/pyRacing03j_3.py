...
class Car:
    ...
    def trigger_random_event(self, is_turning):
        events = [
            ("Lock up", lambda: is_turning and random.random() < 0.05, -30, 5),
            ("Brake overheating", lambda: is_turning and random.random() < 0.01, -10, 20),
            ("Handling malfunction", lambda: random.random() < 0.03, -5, 30),
            ("Gearbox malfunction", lambda: random.random() < 0.01, -25, 40),
            ...
            ("Fuel pressure drop", lambda: random.random() < 0.01, -5, 50),
            ("Fuel flow issue", lambda: self.speed < 150 and random.random() < 0.01, -5, 100),
            ("Inspired", lambda: random.random() < 0.05, 33, 15),
            ("Flustered :(", lambda: random.random() < 0.05, -10, 30),
            ("Adrenaline boost", lambda: random.random() < 0.05, 5, 40),            
...
def render_race_progress(cars, track_length, display_width=100):
    ...
    print(f"{Colors.WHITE}{'Pos':>3} {'Car':^5} {'Driver':^20} {'Best Lap':^10} {'Speed':^10} {'Distance':^12} {'Last 4 Laps':^40}{Colors.RESET}")
    print(f"{Colors.WHITE}{'-'*3} {'-'*5} {'-'*20} {'-'*10} {'-'*10} {'-'*12} {'-'*40}{Colors.RESET}")
    
    for i, car in enumerate(sorted_cars, 1):
        driver_name = f"{car.first_name[:1]}. {car.last_name[:8]}"
        best_lap = f"{car.best_lap_time:.2f}s" if car.best_lap_time != float('inf') else "N/A"
        last_4_laps = " ".join([f"{lap:.2f}" for lap in car.last_4_lap_times])
        print(f"{Colors.WHITE}{i:3d} {Colors.BLUE}{car.symbol:^5} {Colors.WHITE}{driver_name:20s} {best_lap:10s} {Colors.GREEN}{car.speed:10.2f} {Colors.YELLOW}{car.distance:12.2f} {Colors.CYAN}{last_4_laps:40s}{Colors.RESET}")
...
def write_driver_report(car, track_id, results_dir):
    ...
    # Create the filename for this driver's report
...
def show_starting_grid(cars):
    ...
    for i, car in enumerate(cars, 1):
        print(f"{i:2d}. {car.symbol} {car.first_name} {car.last_name}")
    print("=" * 40)
...
def simulate_race(track_sequence, cars, num_laps, time_step=0.1):
    ...
    # Show starting grid and wait for user input
...
def main():
    ...
    # Get the track ID from the filename (assuming it's in the format 'track_X.csv')
    track_id = track_filename.split('_')[1].split('.')[0]

    sorted_cars = simulate_race(track_sequence, cars, num_laps=8)
